...
2. Instant spells occupy a full 1T cast window and show a normal filling cast bar. At the tick boundary, resolve their effects and simultaneous damage first; check deaths before ordinary healing and damage. Instant is resolution priority, not an extra cast or a skipped tick.
...
- Awaken transforms that copy once per duel when its condition is met. Conditions can be reached before its first cast. The replay shows the transformation without extending the tick, then preserves its border and updated text.

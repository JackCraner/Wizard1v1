...
it('keeps every round-one reroll rank one and advances odds by round',()=>{
  ...
  expect(shopRankOdds(1)).toEqual([100,0,0,0,0]);
  expect(shopRankOdds(2)).toEqual([80,20,0,0,0]);
...
it('rejects invalid balancing tables with clear errors',()=>{
  ...
  expect(()=>validateShopOdds([{fromRound:1,rankPercent:[90,10,0,0,0]}])).toThrow('only rank 1');

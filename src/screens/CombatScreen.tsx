...
function Mage({ fighter, opponent, compact, finished, tick, speed, playing, highlights, onInspect }: { fighter: Fighter; opponent?: boolean; compact: boolean; finished:boolean; tick:number; speed:number; playing:boolean; highlights:string[]; onInspect:()=>void }) {
  return <View style={[s.mage, opponent ? { right: '18%', top: 0 } : { left: '13%', bottom: 0 }, { width: compact ? 112 : 190, height: '100%' }]}>
    ...
    <Text numberOfLines={1} style={[s.mageName,{position:'absolute',top:33}]}>{opponent ? fighter.name : 'You'} · Lv {fighter.level} · {fighter.attuned.map((d,i)=><Text key={d} style={{color:DOMAIN_COLORS[d]}}>{i?' + ':''}{d}</Text>)}</Text>
      ...
      <View style={s.field}>
        <View style={s.leftField}>
          <View style={s.arena}>
            ...
            <View style={s.beatNotice}><Text numberOfLines={2} style={s.event}>{finished ? { victory: `Victory · +${session.level} trophies`, defeat: 'Defeat', draw: 'Draw' }[battle.outcome] : frame === 0 ? 'Spells locked · Duel begins' : current.notices?.length?[...new Set(current.notices.map(n=>`${n.side==='player'?'You':'Foe'} · ${n.text}`))].join('\n'):current.messages.join('\n')}</Text></View>
          </View>

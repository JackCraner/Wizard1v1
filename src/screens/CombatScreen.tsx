...
import {continuousCombatFrame} from '../game/playback';
 ...
 return <CombatConnections frame={current} speed={speed}><View style={{flex:1,backgroundColor:'#11151a'}}><Image source={combatArt.background} resizeMode="cover" style={[StyleSheet.absoluteFill,{width:'100%',height:'100%',opacity:.3}]} accessible={false}/><SafeAreaView style={{flex:1}}><View style={{flex:1,padding:compact?8:16,gap:compact?6:12}}>
  ...
  <View style={{flex:1,minHeight:0,flexDirection:'row',gap:compact?22:60,paddingHorizontal:compact?12:70,alignItems:'stretch'}}>
   {(['player','bot'] as const).map(side=>{const u=current[side],oath=u.memory.sequenceOath;return <View key={side} style={{flex:1,minWidth:0,paddingBottom:compact?35:42,gap:3}}>
    ...
    <View style={{flex:1,minHeight:0,flexDirection:'row',gap:6}}><View style={{flex:1}}><EffectBar fighter={u} compact group="buff" onInspect={()=>game.setPaused(true)}/></View><View style={{flex:1}}><EffectBar fighter={u} compact group="debuff" onInspect={()=>game.setPaused(true)}/></View>{u.imp&&<CombatAnchor id={`${side}:imp`} accessibilityLabel={`${side==='player'?'Your':'Opponent'} Imp: ${u.imp.health} of ${u.imp.maxHealth} Health`} style={{width:82,borderRadius:5,padding:4,backgroundColor:'#30233c',gap:2}}><Image source={require('../../assets/Imp.png')} accessible={false} resizeMode="contain" style={{position:'absolute',right:0,top:0,width:34,height:34,opacity:.7}}/><Text style={{color:'#e0bdf3',fontSize:9,fontWeight:'700'}}>IMP {u.imp.health>0?'':'DEFEATED'}</Text><Text style={{color:'#fff0dd',fontSize:10}}>{u.imp.health}/{u.imp.maxHealth}</Text><View style={{height:4,backgroundColor:'#17121d'}}><View style={{height:4,width:`${u.imp.health/u.imp.maxHealth*100}%`,backgroundColor:'#ce9ee7'}}/></View>{current.damageEvents?.some(e=>e.side===side&&e.target==='imp')&&<Text style={{color:'#ff968e',fontSize:11}}>−{current.damageEvents.filter(e=>e.side===side&&e.target==='imp').reduce((n,e)=>n+e.amount,0)}</Text>}</CombatAnchor>}</View>

...
function Mage({ fighter, opponent, compact, finished, tick, speed, playing, highlights, onInspect }: { fighter: Fighter; opponent?: boolean; compact: boolean; finished:boolean; tick:number; speed:number; playing:boolean; highlights:string[]; onInspect:()=>void }) {
  return <View style={[s.mage, opponent ? { right: '18%', top: 0 } : { left: '13%', bottom: 0 }, { width: compact ? 112 : 190, height: '100%' }]}>
    ...
    <Text numberOfLines={1} style={[s.mageName,{position:'absolute',top:33}]}>{opponent ? fighter.name : 'You'} · {fighter.attuned.map((d,i)=><Text key={d} style={{color:DOMAIN_COLORS[d]}}>{i?' + ':''}{d}</Text>)}</Text>
    ...
    <View style={[s.page, { padding: compact ? 7 : 16, gap: compact ? 6 : 12 }]}>
      ...
      <View style={s.field}>
        <View style={s.leftField}>
          ...
          {finished&&<View style={{alignItems:'center'}}><Control label={session.round%2===0?"Choose augment →":"Return to shop →"} disabled={busy} selected onPress={()=>act({type:'next'})} /></View>}
        </View>

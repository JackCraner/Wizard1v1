  ...
  return <View style={s.panel}>
    ...
    <Modal visible={inspected!==null} transparent animationType="fade" onRequestClose={()=>setInspected(null)}><View style={s.shade}><View accessibilityViewIsModal style={s.detail}>
      ...
      <ScrollView contentContainerStyle={{gap:12}}>{inspected!==null&&(['player','bot'] as const).map(side=>{
        ...
        return <View key={side} style={{gap:5}}><Text style={s.detailTitle}>{side==='player'?'You':'Opponent'}</Text>{events.map((e,i)=><View key={i} style={s.detailRow}><SpellIcon id={e.spell} size={32} /><View style={{flex:1}}><Text style={s.detailName}>{SPELLS[e.spell].name}{(e.xp??0)>=3?' · Upgraded':''}</Text><Text style={s.detailText}>{e.status==='skipped'?'Interrupted':`Cast${(e.repeats??1)>1?' · Repeat ×'+e.repeats:''}`}</Text>{e.details?.map((detail,j)=><Text key={j} style={s.detailText}>{detail}</Text>)}</View></View>)}{casting&&<Text style={s.detailText}>Charging {SPELLS[casting.spell].name} · {casting.remaining} ticks remaining</Text>}{!events.length&&!casting&&<Text style={s.detailText}>{reshuffle?`Reshuffling · ${reshuffle} ticks remaining`:battle.frames[inspected-1]?.[side].reshuffleRemaining?'Reshuffle complete. Deck starts next tick.':'No spell cast.'}</Text>}</View>;
      })}{inspected!==null&&<View style={{gap:5}}><Text style={s.detailTitle}>Tick outcomes</Text>{battle.frames[inspected].notices?.map((n,i)=><Text key={`n${i}`} style={s.detailText}>{n.side==='player'?'You':'Opponent'}: {n.text}</Text>)}{battle.frames[inspected].damageEvents?.map((e,i)=><Text key={`d${i}`} style={s.detailText}>{e.side==='player'?'You':'Opponent'}{e.target==='imp'?" · Imp":e.target==='ward'?' · Ward':''}: −{e.amount} {e.target==='ward'?'Ward':'HP'} · {e.kind==='dot'?'damage over time':e.kind==='cost'?'health cost':'spell damage'}{e.critical?' · critical':''}</Text>)}{battle.frames[inspected].healingEvents?.map((e,i)=><Text key={`h${i}`} style={s.detailText}>{e.side==='player'?'You':'Opponent'}{e.target==='imp'?' · Imp':''}: +{e.amount} HP · {e.kind==='hot'?'healing over time':'healing'}</Text>)}</View>}</ScrollView>
    </View></View></Modal>
  </View>;
}
const s=StyleSheet.create({
  panel:{borderWidth:1,borderColor:'#957344',borderRadius:5,backgroundColor:'#20170ef0',paddingHorizontal:7,paddingVertical:4,gap:3},header:{flexDirection:'row',alignItems:'center',gap:8},title:{color:'#e9d3a6',fontSize:10,fontWeight:'600'},sub:{color:'#bca989',fontSize:9},legend:{color:'#a89576',fontSize:8,marginTop:2},pageButton:{padding:5},tracks:{flexDirection:'row'},labels:{width:32},rowLabel:{fontSize:8,color:'#a08e71',textAlignVertical:'center'},column:{flex:1,minWidth:0,borderRightWidth:1,borderRightColor:'#7b603122'},groupEdge:{borderRightColor:'#b4975966'},current:{backgroundColor:'#6d572955',borderWidth:1,borderColor:palette.brass,borderRightColor:palette.brass},tick:{fontSize:8,color:'#b09a76',height:12,textAlign:'center'},cell:{alignItems:'center',justifyContent:'center'},playerCell:{backgroundColor:'#49778718',borderBottomWidth:1,borderBottomColor:'#9a805033'},skip:{position:'absolute',right:0,bottom:0,color:'#dfa492',fontSize:10},count:{position:'absolute',right:0,top:0,color:'#fff0bf',fontSize:7},shade:{flex:1,alignItems:'center',justifyContent:'center',backgroundColor:palette.overlay,padding:20},detail:{width:'100%',maxWidth:450,maxHeight:'95%',padding:18,borderWidth:1,borderColor:'#b4955c',borderRadius:7,backgroundColor:palette.brownSurface,gap:14},detailTitle:{color:'#efd8a9',fontSize:17},detailRow:{flexDirection:'row',gap:14,alignItems:'center'},detailName:{color:'#eee0c6',fontSize:14},detailText:{color:'#c3ad87',fontSize:12,marginTop:4},close:{minHeight:36,paddingHorizontal:12,alignItems:'center',justifyContent:'center',backgroundColor:'#3b3422',borderWidth:1,borderColor:'#a58a55',borderRadius:4},
});

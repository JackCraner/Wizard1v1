...
export function CombatTimeline({battle,frame,activeTick,round,pacing,compact,controls,onInspect}:{battle:Battle;frame:number;activeTick:number;round:number;pacing:string;compact:boolean;controls:ReactNode;onInspect:()=>void}) {
  ...
  return <View style={s.panel}>
    ...
    <Modal visible={inspected!==null} transparent animationType="fade" onRequestClose={()=>setInspected(null)}><View style={s.shade}><View accessibilityViewIsModal style={s.detail}>
      ...
      })}{inspected!==null&&<View style={{gap:5}}><Text style={s.detailTitle}>Tick outcomes</Text>{battle.frames[inspected].notices?.map((n,i)=><Text key={`n${i}`} style={s.detailText}>{n.side==='player'?'You':'Opponent'}: {n.text}</Text>)}{battle.frames[inspected].damageEvents?.map((e,i)=><Text key={`d${i}`} style={s.detailText}>{e.side==='player'?'You':'Opponent'}{e.target==='imp'?" · Imp":''}: −{e.amount} HP · {e.kind==='dot'?'damage over time':e.kind==='cost'?'health cost':'spell damage'}{e.critical?' · critical':''}</Text>)}{battle.frames[inspected].healingEvents?.map((e,i)=><Text key={`h${i}`} style={s.detailText}>{e.side==='player'?'You':'Opponent'}: +{e.amount} HP · {e.kind==='hot'?'healing over time':'healing'}</Text>)}</View>}</ScrollView>
    </View></View></Modal>
  </View>;
}

...
export function CombatDeck({fighter,side,frame,compact,speed,finished,onInspect}:{fighter:Fighter;side:'player'|'bot';frame:CombatFrame;compact:boolean;speed:number;finished:boolean;onInspect:()=>void}) {
 const previous=useRef(frame.tick);const [peels,setPeels]=useState<(CastEvent&{key:string})[]>([]);const [inspected,setInspected]=useState<string|null>(null);const shuffle=useRef(new Animated.Value(0)).current;
 ...
 const upcoming=(shuffling?fighter.spells.slice(0,3):fighter.spells.slice(fighter.cursor+1,fighter.cursor+4)).map((spell,index)=>({spell,depth:index+1})).reverse();
 useEffect(()=>{
 ...
 return <View style={s.root}>
  ...
  <View style={{flex:1,minHeight:0,maxHeight:compact?112:168,width:'100%',alignItems:'center'}}>
   <View style={{height:'100%',aspectRatio:2/3}}>
    {upcoming.map(({spell,depth})=><Animated.View key={depth} style={[StyleSheet.absoluteFill,{zIndex:10-depth,transform:[{translateX:shuffle.interpolate({inputRange:[0,1],outputRange:[direction*depth*10,direction*(depth%2?18:-12)]})},{translateY:-depth*2},{rotate:`${direction*(depth*5-4)}deg`}]}]}><Pressable accessibilityRole="button" accessibilityLabel={`${side==='player'?'Your':'Opponent'} upcoming spell ${depth}: ${SPELLS[spell].name}. Inspect card.`} onPress={()=>{onInspect();setInspected(spell);}} style={StyleSheet.absoluteFill}><SpellCard {...SPELLS[spell]} compact /></Pressable></Animated.View>)}

    {shuffling?<View style={[StyleSheet.absoluteFill,s.reshuffle,{zIndex:11}]}><Text style={s.shuffleIcon}>⟳</Text><Text style={s.timer}>{remaining}T</Text></View>:<Pressable accessibilityRole="button" accessibilityLabel={`${side==='player'?'Your':'Opponent'} deck: ${count} cards left. ${SPELLS[id].name}. Inspect card.`} onPress={()=>{onInspect();setInspected(id);}} style={[StyleSheet.absoluteFill,{zIndex:11,transform:[{rotate:`${-4*direction}deg`}]}]}><SpellCard {...SPELLS[id]} compact /></Pressable>}
  ...
  <Modal transparent visible={inspected!==null} animationType="fade" onRequestClose={()=>setInspected(null)}><View style={s.shade}><View style={s.preview}>{inspected&&<CardPreview card={SPELLS[inspected]} height={compact?245:340} />}<Pressable accessibilityRole="button" onPress={()=>setInspected(null)} style={s.close}><Text style={s.title}>Close</Text></Pressable></View></View></Modal>
 </View>;
}

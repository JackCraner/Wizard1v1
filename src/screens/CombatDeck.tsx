...
export function CombatDeck({fighter,side,frame,compact,speed,finished,onInspect}:{fighter:Fighter;side:'player'|'bot';frame:CombatFrame;compact:boolean;speed:number;finished:boolean;onInspect:()=>()=>void}) {
 ...
 return <View style={s.root}>
  ...
  {!!((fighter.penanceQueued??0)+(fighter.penanceActive??0))&&<View pointerEvents="none" style={{position:'absolute',top:12,zIndex:30,paddingHorizontal:4,paddingVertical:2,borderRadius:3,borderWidth:1,borderColor:'#e6a769',backgroundColor:'#311b16f2'}}><Text accessibilityLabel={'Penance: '+(fighter.penanceActive??0)+' added to this reshuffle, '+(fighter.penanceQueued??0)+' queued for next reshuffle'} style={{fontSize:8,color:'#ffe0ae',fontWeight:'700'}}>{(fighter.penanceActive??0)>0?'Penance +'+fighter.penanceActive+'T':''}{(fighter.penanceQueued??0)>0?' Next +'+fighter.penanceQueued+'T':''}</Text></View>}

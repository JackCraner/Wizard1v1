...
export function CombatDeck({fighter,side,frame,compact,speed,finished,onInspect}:{fighter:Fighter;side:'player'|'bot';frame:CombatFrame;compact:boolean;speed:number;finished:boolean;onInspect:()=>()=>void}) {
 ...
 function inspect(spell:string){if(!restorePlayback.current)restorePlayback.current=onInspect();setInspected(spell);}
 ...
 return <View style={s.root}>
  ...
  <View style={{flex:1,minHeight:0,maxHeight:compact?112:168,width:'100%',alignItems:'center'}}>
   <View style={{height:'100%',aspectRatio:2/3}}>
    {upcoming.map(({spell,depth})=><Animated.View key={depth} style={[StyleSheet.absoluteFill,{zIndex:10-depth,transform:[{translateX:shuffle.interpolate({inputRange:[0,1],outputRange:[direction*depth*10,direction*(depth%2?18:-12)]})},{translateY:-depth*2},{rotate:`${direction*(depth*5-4)}deg`}]}]}><Pressable accessibilityRole="button" accessibilityLabel={`${side==='player'?'Your':'Opponent'} upcoming spell ${depth}: ${SPELLS[spell].name}. Inspect card.`} onPress={()=>inspect(spell)} style={StyleSheet.absoluteFill}><SpellCard {...SPELLS[spell]} compact /></Pressable></Animated.View>)}

    {shuffling?<View style={[StyleSheet.absoluteFill,s.reshuffle,{zIndex:11}]}><Text style={s.shuffleIcon}>⟳</Text><Text style={s.timer}>{remaining}T</Text></View>:<Pressable accessibilityRole="button" accessibilityLabel={`${side==='player'?'Your':'Opponent'} deck: ${count} cards left. ${SPELLS[id].name}. Inspect card.`} onPress={()=>inspect(id)} style={[StyleSheet.absoluteFill,{zIndex:11,transform:[{rotate:`${-4*direction}deg`}]}]}><SpellCard {...SPELLS[id]} compact /></Pressable>}

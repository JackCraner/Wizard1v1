      ...
      <View onLayout={event => setSize(event.nativeEvent.layout)} style={[s.page, { padding: compact ? 6 : 18, gap: compact ? 5 : 14 }]}>
        ...
        <View style={s.dock}>
          <View style={[s.player, { width: tight ? 106 : compact ? 124 : 172 }]}>
            ...
            <AugmentInventory augments={session.augments} compact inline/>
          ...
          <View style={[s.roundActions, { width: tight ? 102 : 134 }]}>
            ...
            <Pressable accessibilityRole="button" accessibilityLabel={excess ? `Battle unavailable: remove ${excess} spell${excess===1?'':'s'}` : session.multiplayer ? 'Ready for battle' : 'Next round'} disabled={busy || !session.spells.length || excess>0} onPress={() => act({ type: 'fight' })} style={({ pressed }) => [s.next, (busy || !session.spells.length || excess>0) && s.disabled, pressed && s.pressed]}><Text style={[s.nextLabel,excess>0&&{fontSize:15,textAlign:'center'}]}>{excess ? `Remove ${excess} spell${excess===1?'':'s'}` : session.spells.length ? session.multiplayer ? 'Ready →' : 'Battle →' : 'Buy a spell'}</Text></Pressable>
          </View>
        </View>

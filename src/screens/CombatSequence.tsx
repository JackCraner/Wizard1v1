...
function MovingTile({id,center,ready,jumpKey,echoShadow,awakening,style,children}:{id:string;center:number;ready:boolean;jumpKey?:string;echoShadow?:ReactNode;awakening?:ReactNode;style:StyleProp<ViewStyle>;children:ReactNode}){
 ...
 return <CombatAnchor id={id}><Animated.View style={{transform:[{translateY:hop},{translateX:offset},{rotate:excited.interpolate({inputRange:[-1,0,1],outputRange:['-0.7deg','0deg','0.7deg']})}],borderRadius:7,boxShadow:ready?'0 0 12px rgba(105,255,224,0.55)':'none'}}>{echoShadow}<Animated.View style={[style,{zIndex:1}]}>{children}</Animated.View>{awakening}{jumpKey&&<View pointerEvents="none" style={{position:'absolute',top:-25,left:0,right:0,alignItems:'center'}}><Text accessibilityLabel="Trigger activated!" style={{color:'#fff2a0',fontSize:30,lineHeight:34,fontWeight:'900',textShadowColor:'#362308',textShadowRadius:5}}>!</Text></View>}</Animated.View></CombatAnchor>;
}

...
function MovingTile({id,center,ready,style,children}:{id:string;center:number;ready:boolean;style:StyleProp<ViewStyle>;children:ReactNode}){
 ...
 return <CombatAnchor id={id}><Animated.View style={{transform:[{translateX:offset},{rotate:excited.interpolate({inputRange:[-1,0,1],outputRange:['-0.7deg','0deg','0.7deg']})}],borderRadius:7,boxShadow:ready?'0 0 12px rgba(105,255,224,0.55)':'none'}}><Animated.View style={style}>{children}</Animated.View></Animated.View></CombatAnchor>;
}
...
export function CombatSequence({fighter,side,frame,compact,speed,playing,onInspect}:{fighter:Fighter;side:'player'|'bot';frame:CombatFrame;compact:boolean;speed:number;playing:boolean;onInspect:()=>()=>void}){
 ...
 return <View style={{gap:4}}>
  ...
  <View onLayout={e=>setRowWidth(e.nativeEvent.layout.width)} style={{height:compact?84:132,flexDirection:'row',justifyContent:'center',gap:6,position:'relative'}}>
   {indices.map(index=>{const c=combatCard(fighter,index),st=fighter.memory.cards?.[index],casting=fighter.casting?.index===index,broken=ghosts.includes(index),cue=cues.find(n=>n.index===index&&['trigger','retrigger','awaken','fragile','oath'].includes(n.status)),target=cues.some(n=>n.status==='retrigger'&&n.targetIndex===index);const trigger=c.combat?.effects?.find(e=>e.kind==='trigger'),ready=!!st?.armed&&!broken&&st.firedCycle!==fighter.cycle&&!(trigger?.onceCombat&&st.firedCombat);const accent=DOMAIN_COLORS[c.domain];return <MovingTile key={index} id={`${side}:spell:${index}`} center={x(index)} ready={ready} style={{height:compact?84:132,width:Math.max(1,cardWidth),borderWidth:casting||st?.awakened?2:1,borderColor:target||cue?.status==='retrigger'?'#efb4ff':cue?.status==='trigger'?'#bffff6':ready?'#69ffe0':st?.awakened?'#e9aaff':casting?'#ffe5a2':accent+'88',borderRadius:7,overflow:'hidden',backgroundColor:broken?'#53262b':ready?'#153b38':casting?'#34312a':'#14212a',opacity:broken?pulse:1,transform:[{scale:cue?.status==='awaken'||cue?.status==='trigger'||target?pulse.interpolate({inputRange:[0,.35,.6,1],outputRange:[1,1.025,1.035,1]}):1}]}}>
    <Pressable accessibilityRole="button" accessibilityLabel={`${side==='player'?'Your':'Opponent'} slot ${index+1}: ${c.name}${casting?', casting':''}${st?.awakened?', Awakened':''}${ready?', Trigger ready':st?.armed?', Trigger spent until next Cycle':trigger?', Trigger arms after first cast':''}${cue?`, ${cue.text}`:''}. ${c.rules}`} onPress={()=>{restore.current=onInspect();setInspect(index);}} style={{flex:1,padding:5,gap:3}}>
     ...
     <Text numberOfLines={2} style={{color:'#fff0d1',fontWeight:'800',fontSize:compact?10:13,lineHeight:compact?12:16}}>{c.name}</Text>
     <Text numberOfLines={compact?1:3} style={{color:'#c5d3d7',fontSize:compact?8:11}}>{c.rules.replace(/\[|\]/g,'')}</Text>
     <Text numberOfLines={1} style={{color:target||cue?.status==='retrigger'?'#efb4ff':cue?.status==='trigger'||ready?'#9ffff0':accent,fontSize:8,fontWeight:'800'}}>{broken?'╱╲ BROKEN':(cue?.status==='trigger'?(target?'ϟ FIRED + RETRIGGER':'ϟ TRIGGERED'):cue?.text)??(target?'↻ RETRIGGERED':ready?`◆ READY · ${({impHurt:'IMP HURT',fatal:'SAVE',echo:'ECHO',heat:'HEAT',poison:'POISON',oath:'OATH',cycle:'CYCLE'}[trigger?.event??'cycle'])}`:st?.armed?'◇ NEXT CYCLE':st?.awakened?'✦ AWAKENED':st?.rules?.length?'✧ ACTIVE RULE':trigger?'◇ ARM ON CAST':casting?'▶ CASTING':'')}</Text>
     {(predicted.heat===index||predicted.tide===index)&&<Text style={{position:'absolute',left:4,right:4,bottom:1,color:'#ffe39a',backgroundColor:'#152027',fontSize:8,fontWeight:'900'}}>{[predicted.heat===index?'♨ EMPOWERED NEXT':'',predicted.tide===index?'≈ ECHO NEXT':''].filter(Boolean).join(' · ')}</Text>}

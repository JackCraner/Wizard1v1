...
function MovingTile({id,center,ready,jumpKey,style,children}:{id:string;center:number;ready:boolean;jumpKey?:string;style:StyleProp<ViewStyle>;children:ReactNode}){
 ...
 return <CombatAnchor id={id}><Animated.View style={{transform:[{translateY:hop},{translateX:offset},{rotate:excited.interpolate({inputRange:[-1,0,1],outputRange:['-0.7deg','0deg','0.7deg']})}],borderRadius:7,boxShadow:ready?'0 0 12px rgba(105,255,224,0.55)':'none'}}><Animated.View style={style}>{children}</Animated.View>{jumpKey&&<View pointerEvents="none" style={{position:'absolute',top:-25,left:0,right:0,alignItems:'center'}}><Text accessibilityLabel="Trigger activated!" style={{color:'#fff2a0',fontSize:30,lineHeight:34,fontWeight:'900',textShadowColor:'#362308',textShadowRadius:5}}>!</Text></View>}</Animated.View></CombatAnchor>;
}
...
function TriggerFlash({order,jumps,speed,playing,armed}:{order:number;jumps:number;speed:number;playing:boolean;armed:boolean}){
 ...
 return <Animated.View pointerEvents="none" style={{position:'absolute',inset:0,borderRadius:6,borderWidth:2,borderColor:armed?'#e6ffb0':'#baffef',backgroundColor:armed?'#bdff5522':'#69ffe044',opacity:reduced?.25:progress.interpolate({inputRange:[-1,-.001,0,.2,1],outputRange:[0,0,.65,.8,0],extrapolate:'clamp'})}}/>;
}

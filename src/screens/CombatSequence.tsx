import {CardShatter,OathLight} from './CardResolutionEffects';
import {CardAwaken} from './CardAwaken';
...
function MovingTile({id,center,ready,jumpKey,echoShadow,awakening,shatter,style,children}:{id:string;center:number;ready:boolean;jumpKey?:string;echoShadow?:ReactNode;awakening?:ReactNode;shatter?:{width:number;height:number;playing:boolean;speed:number;onDone:()=>void};style:StyleProp<ViewStyle>;children:ReactNode}){
 const excited=useRef(new Animated.Value(0)).current,reducedMotion=useCombatMotion();
 useEffect(()=>{excited.setValue(0);if(!ready||reducedMotion)return;const animation=Animated.loop(Animated.sequence([Animated.timing(excited,{toValue:1,duration:900,useNativeDriver:Platform.OS!=='web',isInteraction:false}),Animated.timing(excited,{toValue:-1,duration:160,useNativeDriver:Platform.OS!=='web',isInteraction:false}),Animated.timing(excited,{toValue:1,duration:160,useNativeDriver:Platform.OS!=='web',isInteraction:false}),Animated.timing(excited,{toValue:0,duration:700,useNativeDriver:Platform.OS!=='web',isInteraction:false})]));animation.start();return()=>animation.stop();},[ready,reducedMotion]);
 const hop=useRef(new Animated.Value(0)).current;
 useEffect(()=>{hop.setValue(0);if(!jumpKey||reducedMotion)return;const animation=Animated.sequence([Animated.timing(hop,{toValue:-7,duration:160,useNativeDriver:Platform.OS!=='web',isInteraction:false}),Animated.spring(hop,{toValue:0,friction:4,tension:140,useNativeDriver:Platform.OS!=='web',isInteraction:false})]);animation.start();return()=>animation.stop();},[jumpKey,reducedMotion]);
 const previous=useRef(center),offset=useRef(new Animated.Value(0)).current;
 useEffect(()=>{offset.setValue(previous.current-center);previous.current=center;const animation=Animated.timing(offset,{toValue:0,duration:300,useNativeDriver:Platform.OS!=='web',isInteraction:false});animation.start();return()=>animation.stop();},[center]);
 return <CombatAnchor id={id}><Animated.View style={{transform:[{translateY:hop},{translateX:offset},{rotate:excited.interpolate({inputRange:[-1,0,1],outputRange:['-0.7deg','0deg','0.7deg']})}],borderRadius:7,boxShadow:ready?'0 0 12px rgba(105,255,224,0.55)':'none'}}>{echoShadow}<View style={{zIndex:1}}>{shatter?<CardShatter {...shatter} cardStyle={style}>{children}</CardShatter>:<Animated.View style={style}>{children}</Animated.View>}</View>{awakening}{jumpKey&&<View pointerEvents="none" style={{position:'absolute',top:-25,left:0,right:0,alignItems:'center'}}><Text accessibilityLabel="Trigger activated!" style={{color:'#fff2a0',fontSize:30,lineHeight:34,fontWeight:'900',textShadowColor:'#362308',textShadowRadius:5}}>!</Text></View>}</Animated.View></CombatAnchor>;
}
...
export function CombatSequence({fighter,side,frame,compact,speed,playing,feedbackPlaying=playing,onInspect}:{fighter:Fighter;side:'player'|'bot';frame:CombatFrame;compact:boolean;speed:number;playing:boolean;feedbackPlaying?:boolean;onInspect:()=>()=>void}){
 const [inspect,setInspect]=useState<number|null>(null),[dismissed,setDismissed]=useState<{tick:number;indices:number[]}>({tick:-1,indices:[]}),[rowWidth,setRowWidth]=useState(0);const restore=useRef<(()=>void)|null>(null);const {width,height}=useWindowDimensions();
 const pulse=useRef(new Animated.Value(0)).current;
 const schedule=useMemo(()=>triggerSchedule(frame),[frame]);
 useEffect(()=>setDismissed({tick:frame.tick,indices:[]}),[frame.tick]);
 const cues=(frame.notices??[]).filter(n=>n.side===side&&n.index!==undefined);
 useEffect(()=>{pulse.setValue(1);const a=Animated.timing(pulse,{toValue:0,duration:900/speed,useNativeDriver:false});a.start();return()=>a.stop();},[frame.tick]);
 const ghosts=cues.filter(n=>n.status==='fragile'&&!(dismissed.tick===frame.tick&&dismissed.indices.includes(n.index!))).map(n=>n.index!);
 ...
 return <View style={{gap:4}}>
  ...
  <View onLayout={e=>setRowWidth(e.nativeEvent.layout.width)} style={{height:compact?72:132,flexDirection:'row',justifyContent:'center',gap:6,position:'relative'}}>
   {indices.map(index=>{const c=combatCard(fighter,index),st=fighter.memory.cards?.[index],casting=fighter.casting?.index===index,broken=ghosts.includes(index),cue=cues.find(n=>n.index===index&&['trigger','retrigger'].includes(n.status))??cues.find(n=>n.index===index&&['armed','awaken','fragile','oath'].includes(n.status)),target=cues.some(n=>n.status==='retrigger'&&n.targetIndex===index);const trigger=c.combat?.effects?.find(e=>e.kind==='trigger'),ready=!!st?.armed&&!broken&&st.firedTick!==(frame.stateTick??frame.tick)&&!(trigger?.onceCombat&&st.firedCombat);const selfCue=cues.find(n=>(n.status==='trigger'||n.status==='retrigger')&&(n.status==='retrigger'?n.targetIndex:n.index)===index&&(n.origin?.kind==='cycle'||n.origin?.kind==='spell'&&n.origin.side===side&&n.origin.index===index));const heat=casting&&fighter.casting?.empowered||frame.events.some(e=>e.side===side&&e.index===index&&e.details?.includes('Empowered'));const echo=casting&&fighter.casting?.echoPower!==undefined||frame.events.some(e=>e.side===side&&e.index===index&&e.repeats===2);const accent=DOMAIN_COLORS[c.domain];return <MovingTile key={index} id={`${side}:spell:${index}`} center={x(index)} ready={ready} shatter={broken?{width:Math.max(1,cardWidth),height:compact?72:132,playing:feedbackPlaying,speed,onDone:()=>setDismissed(previous=>({tick:frame.tick,indices:[...(previous.tick===frame.tick?previous.indices:[]),index]}))}:undefined} awakening={cues.some(n=>n.status==='awaken'&&n.index===index)?<CardAwaken key={frame.tick} playing={playing} speed={speed}/>:undefined} echoShadow={frame.events.some(e=>e.side===side&&e.index===index&&e.status==='cast'&&(e.repeats??1)>1)?<CardEcho key={`${frame.tick}:${index}`} card={c} index={index} compact={compact} playing={playing} speed={speed}/>:undefined} jumpKey={selfCue?`${frame.tick}:${selfCue.status}`:undefined} style={{height:compact?72:132,width:Math.max(1,cardWidth),borderWidth:casting||st?.awakened?2:1,borderColor:target||cue?.status==='retrigger'?combatColors.retrigger:heat?combatColors.empowered:echo?combatColors.echo:ready?combatColors.trigger:st?.awakened?combatColors.awakened:casting?combatColors.casting:accent+'88',borderRadius:7,overflow:'hidden',backgroundColor:ready?combatColors.readySurface:casting?combatColors.castingSurface:combatColors.cardSurface,opacity:st?.firedTick===(frame.stateTick??frame.tick)?.82:1,transform:[{scale:!broken&&(cue?.status==='awaken'||target)?pulse.interpolate({inputRange:[0,.35,.6,1],outputRange:[1,1.025,1.035,1]}):1}]}}>
    {!broken&&cues.some(n=>n.index===index&&n.status==='oath'&&n.text==='OATH COMPLETE')&&<OathLight key={'oath-'+frame.tick} playing={feedbackPlaying} speed={speed}/>}
    ...
    </Pressable>{!broken&&cue&&['trigger','retrigger','armed'].includes(cue.status)&&<TriggerFlash key={frame.tick+':'+cue.status+':'+index} order={schedule.find(s=>s.notice===cue)?.order??-1} jumps={schedule.find(s=>s.notice===cue)?.jumps??1} speed={speed} playing={playing} armed={cue.status==='armed'}/>}{!broken&&<Progress fighter={fighter} index={index} tick={frame.tick} speed={speed} playing={playing}/>}
   </MovingTile>;})}

...
export function EffectBar({fighter,compact,group,highlight=[],onInspect}:{fighter:Fighter;compact:boolean;group:'buff'|'debuff';highlight?:string[];onInspect?:()=>void}) {
 ...
 const displayed:Record<string,number>={...fighter.statuses,...(fighter.memory.tidecallerThreshold?{tidecallerThreshold:fighter.memory.tidecallerThreshold}:{}),...(fighter.memory.echoPower?{echoPower:fighter.memory.echoPower}:{}),...Object.fromEntries(['poisonPower','regenerationPower','nextDamage','nextEcho','critDamage'].filter(id=>((fighter.memory as unknown as Record<string,number>)[id]??0)>1||(id==='nextEcho'&&fighter.memory.nextEcho!==undefined)).map(id=>[id,(fighter.memory as unknown as Record<string,number>)[id]])),...(fighter.memory.permanentResilience?{resilience:1}:{})};
 ...
 const statusInfo=(id:string)=>id==='tidecaller'?{...keyword(id),description:`At ${fighter.memory.tidecallerThreshold??5} stacks, your next spell consumes that many stacks and Echoes at ${(fighter.memory.echoPower??.5)*100}% effectiveness.`}:keyword(id);

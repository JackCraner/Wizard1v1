...
export function Leaderboard({lobby,visible,onClose}:{lobby:Lobby;visible:boolean;onClose:()=>void}) {
 const [card,setCard]=useState<string|null>(null);const {height}=useWindowDimensions();
 ...
 return <Modal visible={visible} transparent animationType="fade" onRequestClose={()=>card?setCard(null):close()}><View style={s.shade}><View accessibilityViewIsModal style={s.panel}>
  <View style={s.header}><View style={{flex:1}}><Text style={s.title}>{card?SPELLS[card].name:'Race to the crown'}</Text><Text style={s.muted}>{lobby.players.length} players · First to {lobby.winsToWin} wins · Draws earn no trophies</Text></View><Pressable accessibilityRole="button" onPress={card?()=>setCard(null):close} style={s.button}><Text style={s.text}>{card?'Back to leaderboard':'Close leaderboard'}</Text></Pressable></View>
  {card?<View style={{alignItems:'center'}}><CardPreview card={SPELLS[card]} height={Math.min(300,height-115)} /></View>:<ScrollView contentContainerStyle={{gap:8}}>
   {players.map((p,index)=><View key={p.id} style={[s.row,p.human&&s.you]}>
    ...
    <View style={s.deck}>{p.lastCombatDeck.map((id,i)=><Pressable key={i} accessibilityRole="button" accessibilityLabel={`${p.name}, spell ${i+1}: ${SPELLS[id].name}`} onPress={()=>setCard(id)} style={s.spell}><SpellIcon id={id} size={24}/><Text numberOfLines={2} style={s.spellName}>{i+1}. {SPELLS[id].name}</Text></Pressable>)}</View>
   </View>)}
  </ScrollView>}
 </View></View></Modal>;
}

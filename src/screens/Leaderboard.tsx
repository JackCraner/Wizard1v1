...
export function Leaderboard({lobby,visible,onClose}:{lobby:Lobby;visible:boolean;onClose:()=>void}) {
 ...
 const players=[...lobby.players].sort((a,b)=>b.trophies-a.trophies||a.losses-b.losses||a.id.localeCompare(b.id));
 return <Modal visible={visible} transparent animationType="fade" onRequestClose={()=>card?setCard(null):close()}><View style={s.shade}><View accessibilityViewIsModal style={s.panel}>
  <View style={s.header}><View style={{flex:1}}><Text style={s.title}>{card?SPELLS[card.id].name:'Race to the crown'}</Text><Text style={s.muted}>{lobby.players.length} players · First to {lobby.trophiesToWin} trophies · Draws earn no trophies</Text></View><Pressable accessibilityRole="button" onPress={card?()=>setCard(null):close} style={s.button}><Text style={s.text}>{card?'Back to leaderboard':'Close leaderboard'}</Text></Pressable></View>
  {card?<View style={{alignItems:'center'}}><CardPreview card={cardAt(card.id,card.xp)} height={Math.min(300,height-115)} /></View>:<ScrollView contentContainerStyle={{gap:8}}>
   {players.map((p,index)=><View key={p.id} style={[s.row,p.human&&s.you]}>
    <View style={s.header}><Text style={s.rank}>{index+1}</Text><Text style={[s.text,{flex:1,fontWeight:'700'}]}>{p.name}{p.human?' · You':''}{lobby.winnerIds.includes(p.id)?' · Winner':''}</Text><Text style={s.trophies}>🏆 {p.trophies}/{lobby.trophiesToWin}</Text><Text style={s.muted}>Lv {p.level} · {p.wins}W · {p.losses}L · {p.draws}D</Text></View>

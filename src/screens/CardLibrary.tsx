...
import { CardPreview, KeywordBoxes, SpellCard, RulesText, UpgradeDetails } from '../components/cards/SpellCard';
...
export function CardLibrary({ onClose }: { onClose: () => void }) {
  ...
  return <SafeAreaView style={s.root}>
    ...
    {held && <View pointerEvents="none" style={s.holdShade}><CardPreview card={held} height={height - 32} width={Math.min(width - 32, 560)} /></View>}
    <Modal transparent visible={!!selected} animationType="fade" onRequestClose={() => setSelected(null)}><View style={s.shade}>{selected && <View accessibilityViewIsModal style={[s.detail, { maxHeight: height - 20 }]}><View style={{ width: Math.min(210, (height - 50) * 2 / 3), aspectRatio: 2 / 3 }}><SpellCard {...selected} /></View><View style={{ flex: 1, minHeight: 0, gap: 8 }}><View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' }}><Text style={s.title}>{selected.name}</Text><Pressable accessibilityRole="button" onPress={() => setSelected(null)} style={s.button}><Text style={s.text}>Close</Text></Pressable></View><ScrollView style={{ flex: 1 }} contentContainerStyle={{ gap: 8 }}><Text style={s.text}>{goldCost(selected)} gold · {castLabel(selected.castTicks)}</Text><RulesText card={selected} rules={selected.rules} keywords={selected.keywords} color={palette.parchmentWarm} /><UpgradeDetails card={selected} /><KeywordBoxes keywords={selected.keywords} />{selected.combat?.blockedReason && <Text style={s.caption}>Not yet playable: {selected.combat.blockedReason}</Text>}{selected.notes.length > 0 && <View style={s.note}><Text style={s.noteTitle}>Rules to confirm</Text>{selected.notes.map(note => <Text key={note} style={s.caption}>• {note}</Text>)}</View>}</ScrollView></View></View>}</View></Modal>
  </SafeAreaView>;
}
const s = StyleSheet.create({
  root: { flex: 1, backgroundColor: '#19120d', padding: 10, gap: 7 }, header: { flexDirection: 'row', alignItems: 'center', gap: 12 }, title: { color: '#eed5a1', fontSize: 20, fontWeight: '600' }, text: { color: palette.parchmentWarm, fontSize: 12 }, caption: { color: '#c1aa82', fontSize: 10, lineHeight: 14 }, search: { color: '#eee0c4', width: 175, minHeight: 34, padding: 7, borderWidth: 1, borderColor: '#8f7545', borderRadius: 4, backgroundColor: '#17140fdc' },
  ...
  shade: { flex: 1, padding: 10, alignItems: 'center', justifyContent: 'center', backgroundColor: palette.overlayStrong }, detail: { flexDirection: 'row', gap: 18, width: '100%', maxWidth: 700, padding: 14, backgroundColor: '#20170f', borderWidth: 1, borderColor: '#ba9558', borderRadius: 7 }, note: { padding: 8, backgroundColor: '#443019', borderRadius: 4, gap: 5 }, noteTitle: { fontSize: 11, fontWeight: '600', color: '#e6c78d' }, holdShade: { position: 'absolute', top: 0, bottom: 0, left: 0, right: 0, zIndex: 100, backgroundColor: palette.overlayStrong, alignItems: 'center', justifyContent: 'center' },
});

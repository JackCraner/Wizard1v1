...
export function CastBar({fighter,tick,speed,playing,finished,compact}:{fighter:Fighter;tick:number;speed:number;playing:boolean;finished:boolean;compact:boolean}) {
  ...
  const spell=SPELLS[cast?.spell??fighter.spells[fighter.cursor]];
  let duration=cast?.totalTicks??spell?.castTicks??0;
  if(!cast){
    if(fighter.statuses['next-instant'])duration=0;
    if(fighter.statuses.slowness)duration++;
    if(fighter.statuses.combust)duration=Math.min(1,duration);
  }
  const remaining=cast?.remaining??duration;
  const cost=spell?.mana==='half'?Math.floor(fighter.mana/2):spell?.mana??0;
  const skipped=!cast&&cost>fighter.mana;
  const instant=duration===0;
  const start=finished||skipped?0:instant?1:Math.max(0,(duration-remaining)/duration);
  const end=finished||skipped?0:instant?1:Math.min(1,start+(fighter.statuses.combust?remaining:1)/Math.max(1,duration));
  useEffect(()=>{
    fill.stopAnimation();fill.setValue(start);
    if(!playing||finished||skipped||instant)return;
    const animation=Animated.timing(fill,{toValue:end,duration:COMBAT_TICK_MS/speed,easing:Easing.linear,useNativeDriver:false});
    animation.start();return()=>animation.stop();
  },[tick,speed,playing,finished,start,end,skipped,instant,fill]);
  const label=finished?'Duel complete':spell?.name??'Ready';
  const detail=finished?'':skipped?'No mana':instant?'Instant':`${remaining}T`;

...
export function CastBar({fighter,tick,speed,playing,finished,compact}:{fighter:Fighter;tick:number;speed:number;playing:boolean;finished:boolean;compact:boolean}) {
  ...
  const skipped=!!fighter.instantThisTick||reshuffling||!cast&&cost>fighter.mana;
  ...
  const label=finished?'Duel complete':fighter.instantThisTick?spell.name:reshuffling?'Reshuffling':spell?.name??'Ready';
  const detail=finished?'':fighter.instantThisTick?'Cast · next tick':reshuffling?`${fighter.reshuffleRemaining}T`:skipped?'No mana':instant?'Instant':`${remaining}T`;

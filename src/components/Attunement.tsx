...
import { attunementRanking, DOMAIN_COLORS } from '../game/attunement';
...
export function AttunementPanel({ spells, ages }: { spells: string[]; ages?: number[] }) {
    ...
    return <>
        <Pressable accessibilityRole="button"
            ...
            onPress={() => setOpen(true)} style={{ gap: 3, padding:4,borderRadius:9,borderWidth:1,borderColor:'#56786d',backgroundColor:'#132721' }}>
            <Text style={{ color: '#e8d4a4', fontSize: 9, fontWeight: '800',letterSpacing:.6 }}>ATTUNEMENT  ›</Text>
            <View style={{flexDirection:'row',gap:4}}>{[0,1].map(index=>{const r=ranks[index];return <View key={index} style={{flex:1,alignItems:'center',gap:1,borderRadius:5,borderWidth:1,borderColor:r?DOMAIN_COLORS[r.domain]+'88':'#496058',backgroundColor:r?DOMAIN_COLORS[r.domain]+'14':'#ffffff04',paddingVertical:3}}>
              ...
              {r&&<View style={{position:'absolute',right:1,top:1,borderRadius:5,backgroundColor:'#132721',paddingHorizontal:2}}><Text style={{color:'#f0ebd8',fontSize:9,fontWeight:'800'}}>{r.count}</Text></View>}
            </View>})}</View>
        </Pressable>

...
export function AugmentInventory({ augments, compact = false, onInspect }: {
    augments: string[];
    compact?: boolean;
    onInspect?: () => void;
}) {
    const [open, setOpen] = useState(false);
    return <><Pressable accessibilityRole="button" accessibilityLabel={'Inspect ' + augments.length + ' augments'} onPress={() => { onInspect?.(); setOpen(true); }} style={{ minHeight: 36, padding: 6, borderWidth: 1, borderColor: '#8275a7', borderRadius: 6, backgroundColor: '#262238' }}><Text style={{ color: '#d5c5ff', fontSize: compact ? 10 : 13, fontWeight: '700' }}>✦ {augments.length}{compact ? "" : " Augments"}</Text>{compact && <Text numberOfLines={1} style={{ color: "#d5c5ff", fontSize: 9 }}>Augments</Text>}</Pressable>

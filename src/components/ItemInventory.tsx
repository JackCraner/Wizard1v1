...
export function ItemInventoryView({inventory={},maxVisible=6,size=28,vertical=false,onSelectItem,onInspect}:{inventory?:Inventory;maxVisible?:number;size?:number;vertical?:boolean;onSelectItem?:(id:string)=>void;onInspect?:()=>()=>void}){
 ...
 return <View style={{minHeight:size,flexShrink:1}}>
  <View style={{flexDirection:vertical?'column':'row',flexWrap:vertical?'nowrap':'wrap',gap:5,alignItems:'center'}}>
   {items.slice(0,maxVisible).map(i=><ItemIcon key={i.id} id={i.id} count={i.stacks} size={size} onPress={()=>inspect(i.id)}/>)}

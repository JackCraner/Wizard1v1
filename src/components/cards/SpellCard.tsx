...
export function SpellCard({ compact = false, shop = false, art, ...input }: SpellCardProps) {
    ...
    const tiny = compact && !shop && size.height < 140;
  ...
  <View style={{ minHeight: shop ? 20 : 12, flexShrink: 0, paddingVertical: 2, flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', paddingHorizontal: 6 * scale, backgroundColor: '#10191c' }}><Text style={{ color: palette.gold, fontSize: Math.max(8, 15 * scale), letterSpacing: scale, fontWeight: '900' }}>{'★'.repeat(card.stars)}</Text>{shop && <Text style={{ color, fontSize: 9, fontWeight: '800' }}>{card.domain.toUpperCase()}</Text>}<Text style={{ color, fontSize: Math.max(9, 14 * scale), fontWeight: '900' }}>{tiny&&card.castTicks===0?'⚡':castLabel(card.castTicks)}</Text></View>
  {!shop && <View style={{ height: tiny ? '50%' : '37%', overflow: 'hidden', backgroundColor: color + '22' }}>{artwork ? <Image accessible={false} source={artwork} resizeMode="cover" style={{ width: '100%', height: '100%' }}/> : <Image accessible={false} source={domainArt[card.domain]} resizeMode="contain" style={{ width: "100%", height: "100%" }}/>}
  ...
  <View style={{ minHeight: shop ? 20 : undefined, height: shop || tiny ? undefined : '15%', flex: tiny ? 1 : undefined, flexShrink: 0, paddingVertical: shop ? 2 : 0, justifyContent: 'center', paddingHorizontal: 4 * scale, borderTopWidth: 1, borderTopColor: color + '66' }}><Text numberOfLines={2} adjustsFontSizeToFit minimumFontScale={.8} style={{ color: palette.parchment, fontFamily: serif, fontWeight: '700', fontSize: Math.max(tiny && size.height >= 100 ? 12 : 9, 13 * scale), textAlign: 'center' }}>{card.name}</Text></View>
  {!tiny && <View style={{ flex: 1, backgroundColor: '#e7dfc7', paddingHorizontal: 5 * scale, paddingVertical: 3 * scale, justifyContent: 'center' }}><RulesText card={card} rules={!shop&&size.height<200&&card.rules.length>100?card.rules.split('. ').slice(0,2).join('. ').replace(/\.$/,'')+'. Hold for full effects.':card.rules} keywords={card.keywords} fontSize={shop ? shopRulesSize : Math.max(9, Math.min(14, scale * (card.rules.length > 85 ? 10 : 12)))}/></View>}

...
export function RulesText({ rules, keywords, card, fontSize = 12, color = '#302117' }: {
...
export function SpellCard({ compact = false, shop = false, art, ...input }: SpellCardProps) {
  ...
  <View style={{ height: '14%', flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', paddingHorizontal: 6 * scale, backgroundColor: '#10191c' }}><Text style={{ color: '#ffe19a', fontSize: Math.max(8, 15 * scale), letterSpacing: scale, fontWeight: '900' }}>{'★'.repeat(card.stars)}</Text><Text style={{ color, fontSize: Math.max(9, 14 * scale), fontWeight: '900' }}>{tiny&&card.castTicks===0?'⚡':castLabel(card.castTicks)}</Text></View>
  <View style={{ height: tiny ? '25%' : shop ? '30%' : '37%', overflow: 'hidden', backgroundColor: color + '22' }}>{artwork ? <Image accessible={false} source={artwork} resizeMode="cover" style={{ width: '100%', height: '100%' }}/> : <Image accessible={false} source={domainArt[card.domain]} resizeMode="contain" style={{ width: "100%", height: "100%" }}/>}
   {card.keywords.includes('fragile')&&<View style={{position:'absolute',top:3,left:3,paddingHorizontal:4,paddingVertical:2,borderRadius:3,backgroundColor:'#42252eed',borderWidth:1,borderColor:'#efb1a1'}}><Text style={{fontSize:Math.max(7,9*scale),fontWeight:'900',color:'#ffe2d5'}}>FRAGILE</Text></View>}

...
export function SpellCard({ compact = false, shop = false, art, ...input }: SpellCardProps) {
  ...
  <View style={{ height: '14%', flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', paddingHorizontal: 6 * scale, backgroundColor: '#10191c' }}><Text style={{ color: '#ffe19a', fontSize: Math.max(8, 15 * scale), letterSpacing: scale, fontWeight: '900' }}>{'★'.repeat(card.stars)}</Text><Text style={{ color, fontSize: Math.max(9, 14 * scale), fontWeight: '900' }}>{card.castTicks}T</Text></View>

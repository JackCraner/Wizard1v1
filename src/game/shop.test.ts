...
describe('catalogue shop authority',()=>{
 it('uses catalogue-only loadouts and offers, with star-based purchases',async()=>{
  ...
  const id=s.shop[0]!;s=await g.execute(s.id,s.revision,{type:'buy',spell:id});expect(s.gold).toBe(10-(SPELLS[id].stars+1));
 ...
 it('charges rerolls once and rejects stale commands',async()=>{
  ...
  expect(s.gold).toBe(8);expect(s.shop).not.toEqual(first.shop);expect(s.shop).toEqual(offersFor(1,1,s.spells,s.augments,s.seed).shop);

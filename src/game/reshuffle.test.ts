...
it('presents one complete animated reshuffle tick before casting the next cycle',()=>{
 const battle=simulate(fighter('A',['wrath','wrath']),fighter('B',['riptide']));
 const shuffle=continuousCombatFrame(battle,2);
 expect(shuffle.player.reshuffleRemaining).toBe(1);
 ...
 const resumed=continuousCombatFrame(battle,3);
 ...
 expect(battle.frames[4].events.filter(e=>e.side==='player')).toHaveLength(1);
 // Each fighter shuffles independently; single-card opponents pause earlier.
 expect(continuousCombatFrame(battle,1).bot.reshuffleRemaining).toBe(1);
});

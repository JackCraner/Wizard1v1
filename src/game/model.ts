...
export interface Effect { kind: 'damage'|'loseCurrentHealth'|'selfDamage'|'bothDamage'|'heal'|'mana'|'bothMana'|'status'|'stealMana'|'cleanse'|'manaPerDotStack'|'multiplyHotStacks'|'consumeDots'|'enhance'|'interrupt'|'consumeBurn'; amount?: number; status?: string; target?: 'self'|'enemy'; critWhen?: string[]; perChannel?: boolean; perStatus?: string; perManaSpent?:boolean }
...
export interface Session { spellXp?:number[]; difficulty:Difficulty; lobby:Lobby; id: string; revision: number; round: number; gold: number; spells: SpellId[]; shop: (SpellId|null)[]; rerolls: number; equipmentShop: EquipmentId[]; equipment: Partial<Record<EquipmentSlot,EquipmentId>>; phase: 'shop'|'result'; wins: number; losses: number; battle: Battle|null }
export type Command = { type: 'reroll' } | { type:'buyEquipment';item:EquipmentId } | { type:'buy';spell:SpellId;shopSlot?:number; target?:number } | {type:'merge';from:number;to:number} | { type:'trash';index:number } | { type:'move';from:number;to:number } | { type:'fight' } | { type:'next' };

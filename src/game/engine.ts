...
export function fighter(name: string, spells: SpellId[], modifiers: EquipmentModifier[] = []): Fighter {
  ...
  return { name, spells: [...spells], ...stats, maxHealth: stats.health, maxMana: stats.mana, shield: 0, statuses: {}, cursor: 0, casting: null };
}
...
export function simulate(playerInput: Fighter, botInput: Fighter, seed = RULES.seed): Battle {
  ...
  const applyStatus = (f: Fighter, id: string, amount: number) => {
    ...
    if(id==='hotstreak' && previous<threshold && f.statuses[id]>=threshold) {
      ...
      messages.push(f.name+' gains 3 Combust.');
    }
  };
  ...
  const takeDamage = (f: Fighter, amount: number) => {
    ...
    if(f.health===0 && f.statuses.phoenix>0) {
      f.health=Math.max(1,Math.floor(f.maxHealth/2)); f.mana=Math.floor(f.maxMana/2);
      messages.push(f.name+' is reborn through Phoenix.');
    }
  };
    ...
    for (let step=0; step<RULES.slots && alive(); step++) {
      ...
      for (const side of ['player','bot'] as const) {
        ...
        if (!f.casting) {
          ...
          if (cost>f.mana) {
            ...
            f.cursor=(f.cursor+1)%f.spells.length; spent[side]=true; continue;
          }
          f.mana-=cost;
        ...
        f.casting=null; f.cursor=(f.cursor+1)%f.spells.length;
      ...
      for(const {side,event,tidecaller} of ready) {
        ...
        for(let r=0;r<repeat;r++) for(const effect of effects) {
          ...
          else if(effect.kind==='loseCurrentHealth') { const amount=Math.floor(f.health*value); f.health-=amount;if(amount>0)damageEvents.push({side,amount,critical:false,kind:'cost',domain:spell.domain}); }
          else if(effect.kind==='bothDamage') { const scaled=Math.round(value*damageMultiplier(f,spell.domain)); const amount=event.critical?criticalDamage(scaled,f.statuses.overheat?statusConfig.overheat.critMultiplier:RULES.critMultiplier):scaled; addHit('player',amount,event.critical,spell.domain); addHit('bot',amount,event.critical,spell.domain); }
          else if(effect.kind==='heal') heal(target,value);
          else if(effect.kind==='mana') manaChange[effect.target==='enemy'?other(side):side]+=value;
          else if(effect.kind==='bothMana') { manaChange.player+=value; manaChange.bot+=value; }
          else if(effect.kind==='status') applyStatus(target,effect.status!,value);

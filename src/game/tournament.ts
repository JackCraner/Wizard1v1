...
export function resolveLobbyRound(session:Session,botStates:BotStates) {
 ...
 for(const [i,p] of lobby.players.entries()) {
  ...
  p.lastCombatDeck=[...p.deck];p.lastCombatRound=session.round;
 }
 for(const [aId,bId] of roundPairings(lobby.players.map(p=>p.id),session.round)) {
  ...
  if(b.human)[a,b]=[b,a];
  const battle=simulate(a.human?fighter(a.name,a.deck,equipmentModifiers(session.equipment)):botFighter(a.name,a.deck,botStates[a.id]),b.human?fighter(b.name,b.deck,equipmentModifiers(session.equipment)):botFighter(b.name,b.deck,botStates[b.id]),RULES.seed+session.round*101+lobby.players.indexOf(a)*17);
